...
`/v1/listen` (Nova) mishears, so one key is the whole setup.
...
**Convergence.** `fixed_point` (consecutive takes identical, so the attractor is a
...
independently. The words every final shares that no seed contained are that
...
- `--offline` is a scripted caricature of a recogniser (homophone confusion,
  dropped function words, house-style reformatting) rather than a speech model. It is
